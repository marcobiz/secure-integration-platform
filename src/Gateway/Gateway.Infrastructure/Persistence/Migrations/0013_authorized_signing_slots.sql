-- Authorized signing slots. Preserve the legacy single signing binding while allowing only exact
-- signing identities declared by the granted Published operation's bounded slot collection.
