-- M5 operation-scoped locator resolution. A granted operation may resolve only
-- the exact logical secret/certificate bindings present in its Published definition.
       ...
       SELECT 1
        ...
        WHERE operation ->> 'operationId' = p_operation_id
          AND CASE r.resource_type
                WHEN 'client_certificate' THEN operation -> 'authentication' ->> 'certificateBinding' = p_logical_binding_id
                WHEN 'secret' THEN p_logical_binding_id IN (
                  operation -> 'authentication' ->> 'usernameBinding',
                  operation -> 'authentication' ->> 'passwordBinding',
                  operation -> 'authentication' ->> 'secretBinding')
                ELSE false
              END)
     AND EXISTS (
       SELECT 1
...
-- The broader M5 overload is intentionally removed after the least-privilege
-- replacement exists, leaving no interval without a callable controlled path.
DROP FUNCTION IF EXISTS gateway.resolve_published_provider_locator(uuid,text,text,uuid,uuid,bigint,bytea,uuid,uuid,uuid);
