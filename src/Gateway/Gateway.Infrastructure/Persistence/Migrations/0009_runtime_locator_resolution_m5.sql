-- M5 controlled runtime locator resolution. Runtime may execute this function for
-- one authenticated, granted, Published operation but cannot enumerate locator rows.
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'gateway_locator_owner') THEN
    CREATE ROLE gateway_locator_owner NOLOGIN NOINHERIT;
  END IF;
END $$;

-- CREATE is required only so this NOLOGIN role can own the controlled function.
GRANT USAGE, CREATE ON SCHEMA gateway TO gateway_locator_owner;
GRANT SELECT ON gateway.tenant, gateway.application, gateway.installation,
  gateway.installation_connector_grant, gateway.connector_definition,
  gateway.connector_version, gateway.connector_binding_bundle_version,
  gateway.provider_resource_catalog_version, gateway.provider_resource_locator
  TO gateway_locator_owner;
...
BEGIN
  IF p_connector_slug IS NULL OR p_operation_id IS NULL OR
   ...
   WHERE r.id = p_catalog_id
     ...
     AND EXISTS (
       SELECT 1
        ...
        WHERE resource.value ->> 'ProviderId' = r.provider_id
...
ALTER FUNCTION gateway.resolve_published_provider_locator(uuid,text,text,uuid,uuid,bigint,bytea,uuid,uuid,uuid)
  OWNER TO gateway_locator_owner;
-- Ownership transfer requires CREATE temporarily; the NOLOGIN owner does not
-- retain general object-creation privileges in the application schema.
REVOKE CREATE ON SCHEMA gateway FROM gateway_locator_owner;
REVOKE ALL ON FUNCTION gateway.resolve_published_provider_locator(uuid,text,text,uuid,uuid,bigint,bytea,uuid,uuid,uuid)
  FROM PUBLIC, gateway_admin, gateway_readonly;
GRANT EXECUTE ON FUNCTION gateway.resolve_published_provider_locator(uuid,text,text,uuid,uuid,bigint,bytea,uuid,uuid,uuid)
  TO gateway_runtime;

REVOKE ALL ON gateway.provider_resource_locator FROM PUBLIC, gateway_runtime, gateway_readonly;
