...
public sealed class PostgresAdminDirectoryStore(AdminPostgresDataSource adminDataSource) : IAdminDirectoryStore
{
    ...
    public Task<AdminPage<ApplicationRecord>> ListApplicationsAsync(int offset, int limit, CancellationToken cancellationToken) => QueryAsync<ApplicationRecord>(
        "SELECT id,code,display_name,status,minimum_broker_version,maximum_broker_version,created_at,count(*) OVER() FROM gateway.application ORDER BY code OFFSET $1 LIMIT $2",
    ...
    public Task<AdminPage<GatewayEnvironmentRecord>> ListEnvironmentsAsync(int offset, int limit, CancellationToken cancellationToken) => QueryAsync<GatewayEnvironmentRecord>(
        "SELECT id,code,display_name,production_controls,count(*) OVER() FROM gateway.environment ORDER BY code OFFSET $1 LIMIT $2",
    ...
    public Task<AdminPage<InstallationRecord>> ListInstallationsAsync(Guid tenantId, int offset, int limit, CancellationToken cancellationToken) => QueryTenantAsync<InstallationRecord>(tenantId,
        "SELECT id,tenant_id,application_id,environment_id,status,broker_version,created_at,last_seen_at,revoked_at,revocation_reason,count(*) OVER() FROM gateway.installation WHERE tenant_id=$1 ORDER BY created_at DESC OFFSET $2 LIMIT $3",
    ...
    public Task<AdminPage<InstallationGrantRecord>> ListGrantsAsync(Guid tenantId, int offset, int limit, CancellationToken cancellationToken) => QueryTenantAsync<InstallationGrantRecord>(tenantId,
        "SELECT g.id,g.installation_id,g.tenant_id,c.slug,g.operation_id,g.enabled,g.valid_from,g.valid_until,count(*) OVER() FROM gateway.installation_connector_grant g JOIN gateway.connector_definition c ON c.id=g.connector_id WHERE g.tenant_id=$1 ORDER BY c.slug,g.operation_id OFFSET $2 LIMIT $3",
    ...
    public Task<AdminPage<GatewayAuditEvent>> ListAuditAsync(Guid tenantId, int offset, int limit, CancellationToken cancellationToken) => QueryTenantAsync<GatewayAuditEvent>(tenantId,
        "SELECT id,occurred_at,tenant_id,actor_type,actor_id,action,target_type,target_id,correlation_id,outcome,reason_code,metadata_redacted::text,count(*) OVER() FROM gateway.audit_event WHERE tenant_id=$1 ORDER BY occurred_at DESC OFFSET $2 LIMIT $3",
    ...
    private async Task<AdminPage<T>> QueryAsync<T>(string sql, Func<NpgsqlDataReader, T> read, int offset, int limit, CancellationToken cancellationToken)
    {
        ValidatePage(offset, limit);
        List<T> values = []; int total = 0;
        await using NpgsqlConnection connection = await dataSource.OpenConnectionAsync(cancellationToken).ConfigureAwait(false);
        await using NpgsqlCommand command = new(sql, connection);
        command.Parameters.AddWithValue(offset); command.Parameters.AddWithValue(limit);
        await using NpgsqlDataReader reader = await command.ExecuteReaderAsync(cancellationToken).ConfigureAwait(false);
        while (await reader.ReadAsync(cancellationToken).ConfigureAwait(false)) { values.Add(read(reader)); total = checked((int)reader.GetInt64(reader.FieldCount - 1)); }
        return new(values, offset, limit, total);
    }

    private async Task<AdminPage<T>> QueryTenantAsync<T>(Guid tenantId, string sql, Func<NpgsqlDataReader, T> read, int offset, int limit, CancellationToken cancellationToken)
    {
        ValidatePage(offset, limit);
        List<T> values = []; int total = 0;
        await using NpgsqlConnection connection = await dataSource.OpenConnectionAsync(cancellationToken).ConfigureAwait(false);
