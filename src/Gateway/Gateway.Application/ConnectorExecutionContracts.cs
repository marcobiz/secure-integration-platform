...
internal sealed class ConnectorExecutionStrategyRegistry
{
    ...
    private readonly Dictionary<ConnectorExecutionStrategyKey, IConnectorExecutionStrategy> strategies;

    internal ConnectorExecutionStrategyRegistry(IEnumerable<IConnectorExecutionStrategy> values)
    {
        ArgumentNullException.ThrowIfNull(values);
        Dictionary<ConnectorExecutionStrategyKey, IConnectorExecutionStrategy> registered = [];
        foreach (IConnectorExecutionStrategy strategy in values)
        {
            if (strategy is null || strategy.Key is null)
                throw new InvalidOperationException("Connector execution strategy registration is invalid.");
            if (registered.Count >= MaximumStrategies)
                throw new InvalidOperationException("Connector execution strategy registry is full.");
            if (!registered.TryAdd(strategy.Key, strategy))
                throw new InvalidOperationException("Duplicate Connector execution strategy key.");
        }
        strategies = registered;
    }

    internal IConnectorExecutionStrategy Required(ConnectorExecutionStrategyKey key) =>
        strategies.TryGetValue(key, out IConnectorExecutionStrategy? strategy)
            ? strategy
