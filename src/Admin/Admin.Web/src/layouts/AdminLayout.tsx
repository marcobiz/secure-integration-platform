...
export function AdminLayout({ children }: { children: ReactNode }) {
  ...
  const navigation = <Box component="nav" aria-label={t('menu')}><Toolbar><ShieldCheck aria-hidden /><Typography variant="subtitle1" sx={{ ml: 1, fontWeight: 700 }}>{t('product')}</Typography></Toolbar><Divider /><List component="div"><ListItemButton component={RouterLink} to="/" onClick={event => { event.preventDefault(); go('/'); }} selected={currentPath === '/'}><ListItemIcon><Gauge /></ListItemIcon><ListItemText primary={t('dashboard')} /></ListItemButton>{groups.map(group => <Box key={group.label}><Typography variant="overline" sx={{ display: 'block', px: 2, pt: 2, color: 'text.secondary' }}>{t(group.label)}</Typography>{group.items.filter(([label]) => label !== 'access' || hasRole(session, 'SecurityAdministrator')).map(([label, path, Icon]) => <ListItemButton key={path} component={RouterLink} to={path} onClick={event => { event.preventDefault(); go(path); }} selected={currentPath.startsWith(path)}><ListItemIcon><Icon size={20} /></ListItemIcon><ListItemText primary={t(label)} /></ListItemButton>)}</Box>)}</List></Box>;
