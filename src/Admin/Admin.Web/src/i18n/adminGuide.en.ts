  ...
  sections: [
    ...
    { id: 'installations', title: 'Installations and activation handoff', navigationKey: 'installations', route: '/installations', topics: [
      { id: 'installation-create', title: 'Select the client identity', paragraphs: [
        ...
        'SecurityAdministrator creates an Installation by selecting Tenant, Application, Environment and kind, then Create installation. Broker is the Local Broker path; Direct is a client that connects directly to the Gateway. Guided onboarding creates Direct Installations. Existing Environment choices come from the server; choosing an Environment for creation does not move an existing Installation.'
      ] },
    ...
    { id: 'onboarding', title: 'Guided onboarding', navigationKey: 'guidedOnboarding', route: '/onboarding', topics: [
      { id: 'onboarding-prerequisites', title: 'Prepare the supported path', paragraphs: [
        ...
        'Current state, Missing prerequisite, Required role and Next action describe the selected target. Select resources by their displayed names using the paged selectors. The page carries the target in its URL; an authorized colleague can reopen that URL and resume their role’s step. The URL contains administrative identifiers, not enrollment material, and does not grant authority. Keep handoffs within authorized colleagues.',
      ...
      { id: 'onboarding-five-actions', title: 'Five actions across three roles', steps: [
        'SecurityAdministrator: select Tenant, Application and Environment and create a Direct Installation, or select an existing one. Deliver the activation handoff and have the client operator complete enrollment. Continue configuration only when that Installation is Active.',
      ...
      { id: 'onboarding-resume', title: 'Resume, missing catalogs and final verification', paragraphs: [
        ...
        'If a catalog has no eligible choice, ask the deployment owner to provision or correct that resource. There is no provider bootstrap screen here. If a resource revision or binding changed, refresh and review the current authoritative configuration before a new approval. Do not solve stale authority by inventing identifiers or bypassing review. Large catalogs and histories are bounded; if an expected record is unavailable in the guided choices, use the supported administrative surface to inspect it rather than infer absence.',
        'The completion banner confirms Published version state. Independently verify the selected Installation is Active and the intended grants exist. The page does not execute a runtime request and cannot certify external-service readiness. Finish with one bounded invocation through the supported client/runtime interface and inspect metadata-only audit; payload entry and a live invocation runner are not provided in this page.'
      ] }
    ] },
