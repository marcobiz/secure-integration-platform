...
export function ErrorState({ error, retry }: { error: unknown; retry?: () => void }) {
  ...
  return <Alert ref={summary} tabIndex={-1} role="alert" aria-live="assertive" severity="error" action={retry && <Button color="inherit" onClick={retry}>{t('retry')}</Button>}><Typography>{message}</Typography>{problem?.correlationId && <Typography variant="caption">{t('correlation')}: {problem.correlationId}</Typography>}</Alert>;
}
