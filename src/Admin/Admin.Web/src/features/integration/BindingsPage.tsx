...
export function BindingsPage() {
  ...
  return <>
    ...
    <Card><CardContent><Typography id="binding-help" color="text.secondary" sx={{ mb: 2 }}>{t('bindingsHelp')} Submit the complete binding set atomically; only logical names, HTTPS endpoints and opaque provider references are accepted.</Typography><form onSubmit={form.handleSubmit(value => save.mutate(value))} noValidate><Stack spacing={2}>
      <TextField label={t('connectors')} slotProps={{ htmlInput: { 'aria-describedby': 'binding-help' } }} {...form.register('connectorId', { required: true })} error={Boolean(form.formState.errors.connectorId)} helperText={form.formState.errors.connectorId ? 'Connector is required.' : undefined} />
      <TextField label={t('version')} {...form.register('connectorVersion', { required: true })} error={Boolean(form.formState.errors.connectorVersion)} helperText={form.formState.errors.connectorVersion ? 'Version is required.' : undefined} />
      <TextField label={t('environment')} {...form.register('environmentId', { required: true })} error={Boolean(form.formState.errors.environmentId)} helperText={form.formState.errors.environmentId ? 'Environment is required.' : undefined} />
      <TextField multiline minRows={3} label="Endpoint bindings (JSON object)" {...form.register('endpointsJson', { required: true, validate: value => { try { parseBindingMap(value, 'Endpoints'); return true; } catch (error) { return (error as Error).message; } } })} error={Boolean(form.formState.errors.endpointsJson)} helperText={form.formState.errors.endpointsJson?.message ?? 'Example: {"primary":"https://vendor.example/api","secondary":"https://backup.example/api"}'} />
      <TextField multiline minRows={3} label="Secret references (JSON object)" {...form.register('secretReferencesJson', { required: true, validate: value => { try { parseBindingMap(value, 'Secret references'); return true; } catch (error) { return (error as Error).message; } } })} error={Boolean(form.formState.errors.secretReferencesJson)} helperText={form.formState.errors.secretReferencesJson?.message ?? 'Opaque server-side provider references; secret values are never shown.'} />
      <TextField multiline minRows={3} label="Certificate references (JSON object)" {...form.register('certificateReferencesJson', { required: true, validate: value => { try { parseBindingMap(value, 'Certificate references'); return true; } catch (error) { return (error as Error).message; } } })} error={Boolean(form.formState.errors.certificateReferencesJson)} helperText={form.formState.errors.certificateReferencesJson?.message ?? 'Opaque server-side certificate references; private keys are never shown.'} />
      <Button type="submit" variant="contained">{t('save')}</Button>
    </Stack></form>
    {(Object.keys(form.formState.errors).length > 0 || save.error) && <Alert ref={summary} tabIndex={-1} severity="error" sx={{ mt: 2 }}>Binding set was not saved. Correct the identified fields and retry.{save.error && <ErrorState error={save.error} />}</Alert>}
