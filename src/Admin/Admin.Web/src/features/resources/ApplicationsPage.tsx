...
export function ApplicationsPage() {
  const { t } = useTranslation(); const session = useSession(); const cache = useQueryClient(); const [mode, setMode] = useState<'closed' | 'create' | 'edit'>('closed'); const [selected, setSelected] = useState<Application>(); const [current, setCurrent] = useState<Application>(); const [conflictAction, setConflictAction] = useState<'update' | 'disable'>('update'); const [offset, setOffset] = useState(0);
  ...
  const invalidate = async () => { setMode('closed'); setSelected(undefined); setCurrent(undefined); setConflictAction('update'); form.reset(); await cache.invalidateQueries({ queryKey: ['applications'] }); };
  const captureConflict = async (error: Error, row: Application, action: 'update' | 'disable') => {
    if (!(error instanceof ApiProblem) || (error.status !== 409 && error.status !== 412)) return;
    setSelected(row); setConflictAction(action);
    if (action === 'disable') {
      form.reset({ code: row.code, displayName: row.displayName, minimumBrokerVersion: row.minimumBrokerVersion, maximumBrokerVersion: row.maximumBrokerVersion ?? '' });
      setMode('edit');
    }
    setCurrent(await adminApi.application(row.id));
  };
  const create = useMutation({ mutationFn: adminApi.createApplication, onSuccess: invalidate });
  const update = useMutation({ mutationFn: (value: ApplicationForm) => adminApi.updateApplication(selected!.id, { displayName: value.displayName, minimumBrokerVersion: value.minimumBrokerVersion, maximumBrokerVersion: value.maximumBrokerVersion || null }, selected!.rowVersion), onSuccess: invalidate, onError: error => captureConflict(error, selected!, 'update') });
  const disable = useMutation({ mutationFn: (row: Application) => adminApi.disableApplication(row.id, row.rowVersion), onSuccess: invalidate, onError: (error, row) => captureConflict(error, row, 'disable') });
  ...
  const beginEdit = (row: Application) => { update.reset(); disable.reset(); setSelected(row); setCurrent(undefined); setConflictAction('update'); form.reset({ code: row.code, displayName: row.displayName, minimumBrokerVersion: row.minimumBrokerVersion, maximumBrokerVersion: row.maximumBrokerVersion ?? '' }); setMode('edit'); };
  ...
  return <><PageTitle title={t('applications')} action={hasRole(session, 'SecurityAdministrator') ? <Button variant="contained" onClick={beginCreate}>{t('addApplication')}</Button> : undefined} /><DataTable rows={query.data.items ?? []} columns={columns} label={t('applications')} /><PaginationControls page={query.data} onOffset={setOffset} />
    <Dialog open={mode !== 'closed'} onClose={() => setMode('closed')}><form onSubmit={form.handleSubmit(value => mode === 'create' ? create.mutate(value) : update.mutate(value))} noValidate><DialogTitle>{mode === 'create' ? t('addApplication') : t('editApplication')}</DialogTitle><DialogContent><Stack spacing={2} sx={{ mt: 1 }}>
      {current && selected && <Alert severity="warning" action={<Button onClick={() => beginEdit(current)}>{t('reloadCurrent')}</Button>}><Typography>{t('concurrencyConflict')}</Typography><Typography><strong>{t('conflictAction')}:</strong> {t(conflictAction === 'disable' ? 'conflictActionDisable' : 'conflictActionUpdate')}</Typography><ConflictComparison localRowVersion={selected.rowVersion} serverRowVersion={current.rowVersion} fields={[
        ...
        { label: t('status'), localValue: selected.status, serverValue: current.status }
      ]} /></Alert>}
