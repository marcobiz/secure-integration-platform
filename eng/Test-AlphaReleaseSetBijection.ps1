...
function New-ContainerSbom {
    ...
    return [ordered]@{
        ...
        packages = @([ordered]@{
            ...
            copyrightText = 'Copyright 2026 ApoCert S.r.l.'
...
function New-SyntheticReleaseSet {
    ...
    $sbomEntries = @(Get-ChildItem -LiteralPath (Join-Path $RunDirectory 'sbom') -File | Sort-Object Name | ForEach-Object {
        $record = Get-FileRecord -RunDirectory $RunDirectory -RelativePath ('sbom/' + $_.Name) -Kind 'spdx-or-aggregate'
        $record['subjectLicenseExpression'] = if ($_.Name -ceq 'sdk-dotnet.spdx.json') { 'Apache-2.0' } elseif ($_.Name -ceq 'aggregate-manifest.json') { 'MPL-2.0 AND Apache-2.0' } else { 'MPL-2.0' }
        $record
    })
    $manifest = [ordered]@{
        schemaVersion = 1
