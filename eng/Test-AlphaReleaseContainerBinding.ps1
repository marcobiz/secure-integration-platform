...
function New-ReleaseFixture {
    ...
    $manifest = [ordered]@{
        schemaVersion = 2; product = 'SecureIntegrationPlatform'; version = $productVersion; sourceRevision = $ExpectedSourceCommit; releaseChannel = 'public-technical-preview'; releaseClass = 'PUBLIC TECHNICAL PREVIEW'; distributionTarget = 'GitHub public prerelease v0.1.0-alpha.1'
        generatedAtUtc = '2026-01-01T00:00:00.0000000+00:00'; publication = [ordered]@{ state = 'pre-publication-candidate'; occurred = $false; candidateManifestName = 'manifest.json'; publicManifestName = 'release-manifest.json'; checksumsName = 'SHA256SUMS'; integrityClosure = 'sidecar-or-publication-attestation-required'; publicSbomAssets = @('gateway-container.spdx.json','migrations-container.spdx.json'); internalEvidenceSboms = @('sbom/admin-frontend.spdx.json','sbom/aggregate-manifest.json','sbom/auth-certificate-signing.spdx.json','sbom/broker.spdx.json','sbom/connector-cli.spdx.json','sbom/gateway-container.spdx.json','sbom/gateway.spdx.json','sbom/migrations-container.spdx.json','sbom/sdk-dotnet.spdx.json') }; claims = [ordered]@{ productionReady = $false }
