...
| TM-075 | S/T/I/E | Un adapter esterno sceglie ID/type o provider locator, usa un valore caller-controlled come input privilegiato, richiede un input non approvato, conserva plaintext o raggiunge provider/store/DI. | ID/type derivano solo dal profilo Published e risolvono exact-one senza fallback. Nomi richiesti statici bounded vengono frozen a startup e devono coincidere esattamente con mapping Published verso binding `opaque`; Core risolve locator/value e concede solo `WriteRequiredXmlValue` durante la chiamata XML sincrona, poi azzera i buffer. Registrar module-owned e constructor graph negano provider/store/service locator. Binding e revisioni entrano in dependency set, four-eyes digest, resource stamp e locator SQL operation-scoped. | Il provider e il modulo allowlisted restano nella TCB; il valore necessario esiste temporaneamente nei buffer Core e nel request body. Administrator/SYSTEM o un modulo malevolo full-trust possono osservare memoria. |
| TM-076 | S/T/E | Un modulo usa la capability di firma come oracle scegliendo bytes, algoritmo, chiave, certificato, purpose, Tenant/Connector/version o claim non approvate, oppure esporta e riusa il token. | Il bridge accetta soltanto claim scalar bounded; policy, allowlist, RS256, key binding, purpose, catalog revision, SPKI, x5c, subject e lifetime provengono dalla stessa Published A. Il risultato è non costruibile, senza token/property pubblica, reference-bound allo stesso bridge e consumabile soltanto dal transport autorizzato. Primitive M6 rivalidano policy/binding/public material prima del risultato. | Il protocollo remoto riceve necessariamente il bearer firmato. Compromissione del Gateway/provider o reflection full-trust resta fuori dal boundary; replay lato remoto richiede le difese/jti/TTL già qualificate. |
| TM-077 | S/T/E/I | Un modulo trasforma il transport bridge in authenticated HTTP generico scegliendo URI, metodo, certificate, Authorization/header o destinazione privata, oppure sostituisce endpoint/certificato dopo DNS. | Il request pubblico contiene solo body bounded e token opaco same-bridge. Endpoint, metodo, content type, Bearer, correlation, timeout, response bound e certificato mTLS derivano dalla Published A. Sender qualificato applica HTTPS, DNS/rebinding/private-destination policy e confronta nuovamente policy/binding dopo DNS immediatamente prima del transport. Nessun `HttpClient`, `HttpRequestMessage`, URI o header bag è esposto. | Una pubblicazione/revoca dopo l'inizio del dispatch non ritira byte già inviati. Network, trust roots, provider e processo Gateway restano nella TCB. |
...
sulla view XML-only e sull'E2E caller-spoof con valore provider-owned. `SEC-W1-CAP-002` mappa
`TM-076` sul public API inventory senza token/key/certificate selector, sul denied-claim zero-network,
sull'E2E RS256/x5c e sulle matrici M6 wrong-key/purpose/policy/replay. `SEC-W1-CAP-003` mappa
`TM-077` sull'assenza di URI/header/certificate nel request pubblico, sull'endpoint substitution,
sul vero listener mTLS e su `Wave1_MTLS_disable_after_DNS_and_before_transport_causes_zero_dispatch`.
