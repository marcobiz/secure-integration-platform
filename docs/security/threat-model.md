...
| TM-065 | S/T/E | Un caller sceglie adapter, QName, SOAP version, endpoint o valori security del bootstrap, oppure un adapter registrato diverso viene sostituito al profilo approvato. | Resolver accetta solo invocation gia autorizzata e logical profile ID; profilo Typed request+response unico nel Connector Published; exact adapter ID/type, QName, action, endpoint, bounds, ConnectorVersion/checksum/revision/resource stamp nel fingerprint e nel digest four-eyes; registry exact senza reflection; request context immutabile server-derived e nessun business dictionary/callback/template. | Publisher, registry composition e processo Gateway restano nella TCB; un adapter compilato malevolo richiede compromissione della supply chain approvata. |
| TM-066 | T/I/D | XML handshake nested usa DTD/entity, namespace/body confusion, duplicati, ordine/cardinalita errati, mixed content o output adapter illimitato per provocare parsing ambiguo, leakage o consumo memoria. | Writer Core byte-bounded durante la scrittura; reader con DTD proibito, resolver nullo, entity/document/depth/node/attribute limits; Envelope/Body, payload cardinality e QName esatti prima dell'adapter; adapter sintetico nega ordine, cardinalita, domain, nested, duplicati, unexpected e mixed content; outcome chiuso senza raw XML/XElement/dictionary. | La semantica nested di ogni futuro protocollo resta responsabilita dell'adapter compilato e richiede test negativi propri. |
| TM-067 | S/T/I/R/D | Un candidate esterno viene iniettato come business input, fissato o riusato con intent errato/scaduto/cross-context, promosso dopo rotate/disable, conservato in stato illimitato o esposto in diagnostica. | Boundary dedicato `ExternalSessionCandidate` owned/zeroed; provenance chiusa; intent opaco TTL/single-use legato a Tenant/Application/Installation, exact cache key, ConnectorVersion/operation/profile, revisions/checksum/resource stamp; stesso cache cap 256/lazy sweep/striped locks/current generation; validator senza cache API; revalidation policy/resource dopo remote await e promotion atomica solo Core; remote expiry futura e capped; canary redaction. | Memoria e composition root Gateway restano nella TCB; revoca successiva a un dispatch gia iniziato non ritira byte gia trasmessi. |
...
`SEC-W1-HS-001` mappa `TM-065` sui test Published adapter ID/type, nested request,
four-eyes digest, cross-context/profile e public API senza selector adapter/endpoint.
`SEC-W1-HS-002` mappa `TM-066` sulle matrici strict nested response, DTD/QName/Body,
request byte bound e real HTTPS typed handshake.
`SEC-W1-HS-003` mappa `TM-067` sui test wrong/reused/expired intent,
Tenant/Application/Installation binding, validator failure, remote expiry, rotate/disable race,
256-entry lazy sweep, candidate zeroing/redaction e real HTTPS external promotion.
