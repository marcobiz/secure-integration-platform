...
| TM-069 | T/E | Un Connector o tenant induce il caricamento di un modulo non autorizzato, traversal/search di assembly o dipendenza verticale inversa nel Core. | Allowlist deployment-only con path assoluto canonico, assembly full name, type e module ID esatti; path/type/ID unici e bounded; nessuna directory/AppDomain discovery, download o reload; registrar ristretto a tipi module-owned; architecture e Core-export gate negano dipendenze inverse. | Un modulo allowlisted è codice full-trust in-process; ACL, provenance e review del deployment devono proteggere i byte, e Administrator/SYSTEM resta rischio residuo. |
| TM-070 | S/T/I | Una strategy costruisce o muta il contesto, auto-asserisce identity/grant, modifica il payload dopo l'autorizzazione o reinterpreta l'autenticazione inbound. | `AuthorizedConnectorExecution` public-read/internal-construct, senza factory/setter; identity e operation derivano dal principal e catalogo Core; payload bounded copiato e stream non scrivibile senza buffer pubblico; contratto strategy senza raw inbound credential/header/principal mutabile e synthetic assembly senza friend access o `AuthenticateAsync`. | Una strategy allowlisted è trusted in-process e memory corruption/reflection privilegiata non sono sandboxate; review del modulo resta obbligatoria. |
| TM-071 | I/R/D | Una strategy propaga message/inner/stack/provider data o usa una falsa `OperationCanceledException` per simulare la cancellazione del caller. | Boundary generica converte ogni eccezione inattesa in `BGW-EGRESS-UPSTREAM-REJECTED` senza inner/message; conserva cancellation solo quando il token effettivo risulta cancellato; risultato, content type e body restano bounded e copiati prima dell'audit. | Un modulo full-trust può bloccare il processo o eludere garanzie in-process; isolamento di codice non fidato richiederebbe un processo separato ed è fuori scope. |
