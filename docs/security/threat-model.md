...
| TM-063 | S/T/E | Il caller promuove P.IVA, payload o `person_id` a subject FSE, oppure sostituisce organizzazione, ruolo, purpose o action. | Il request contract non espone subject/endpoint/auth selector; `PublishedConnectorFse2ProfileResolver` usa `PublishedConnectorAccessContext`, lifecycle/version/operation/checksum exact e approval four-eyes exact-digest con principal distinti; il resolved profile non è costruibile dal caller; P.IVA/authority producono CX `FixedSubject`; matrice DAP/purpose/action frozen. Test `CONNECTOR_SECURITY_PATH_real_Published_*`, grant denial e invalidated approval verificano provider/network zero ove applicabile. | Publisher/approver collusi o store/host privilegiato restano nella TCB; il profilo human actor non è implementato. |
| TM-064 | T/E | I due JWT, firma, mTLS o endpoint appartengono a revisioni diverse; catalog/provider sostituisce stabilmente una risorsa approvata; CN ambiguo cambia issuer. | Signing e mTLS derivano dallo stesso snapshot Published e portano catalog/binding revision+checksum, provider reference, fingerprint/SPKI/version; `Fse2DispatchLease` serve entrambi i JWT; final transport rivalida l'intera authority dopo la risoluzione mTLS e prima di proiettare gli header/inviare. CN è letto dal DER X.500 richiedendo esattamente un OID `2.5.4.3`. Test production-path coprono sostituzione stabile/cross-use e quattro race TCS con network zero; test CN coprono assenza, duplicati, multi-valued, empty, malformed e SAN-only. | Trust/revocation e custody di certificati ufficiali richiedono qualifica/accreditamento separato. |
| TM-065 | T/I/R | CX/XON/OID ambiguo, backing payload mutato dopo validazione, workflow cross-context riusato o RFC7807 espone payload clinico. | OID validato per archi ASN.1 canonici; snapshot privato unico a ingresso invocation alimenta validation/hash/body; getter copy-on-read; workflow key include Tenant/Application/Installation/Environment/ConnectorVersion/profile revision+checksum/origine e persiste solo identificatori tecnici; response allowlist e stable safe code. Test production-path muta input/getter durante await signing e confronta hash+body; test workflow negano tenant/application/installation/environment/profile cross-use con network zero. | Dump privilegiati e telemetria futura restano soggetti a nuova review; nessun payload clinico è persistito dal pack. |
| TM-066 | E/D | Endpoint caller-controlled, host suffix/porta/path ambiguo, synthetic promosso Production, FHIR test-only promosso in produzione o retry di mutazioni causa esfiltrazione/duplicazione. | Production e OfficialTest accettano solo origin, porta default e base path ufficiali congelati; synthetic richiede capability interna test-only; path/method sono frozen; final lease include endpoint binding revision/checksum; restricted transport conserva DNS/IP, no redirect/proxy, timeout e response bound; solo workflow/trace sono `SAFE_RETRY`. Test production-path coprono attacker endpoint, endpoint race e test-only in Production con provider/network zero. | Disponibilità upstream e accreditamento ufficiale non sono provati dal server sintetico. |
