...
| TM-054 | S/T/I/E | Una sessione opaque viene proiettata in un header controllato dal caller, iniettato con CR/LF, riusato per altra operation/destinazione o inviato stale dopo disable/rotate. | Policy tipizzata server-owned da Published ConnectorVersion/operation/profile/binding; field-name token con denylist security/hop-by-hop; nessun header bag o attach API; cache key operation-bound; lease generation/expiry; stamp credential/binding/endpoint e policy rivalidati dopo await; dispatch one-shot via `IRestrictedTransport`; race test con zero dispatch. | La memoria e la composition root del Gateway restano nella TCB; una revoca che avviene dopo l'inizio effettivo del network dispatch non può ritirare byte già trasmessi. |
