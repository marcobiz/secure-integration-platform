...
| TM-063 | S/T/I/E | Un caller o una configurazione stale combina Basic, SOAPAction/Content-Type e sessione opaque appartenenti a operation, endpoint o revisioni diverse, duplica Authorization/SOAPAction o colloca la sessione in un header infrastrutturale. | Handoff non costruibile; resolver unico da principal/grant e snapshot Published; schema chiuso `soapBasicOpaqueSession`; metadata SOAP tipizzati; Basic unico; denylist custom-header include Authorization/SOAPAction/Content-Type, hop-by-hop, proxy, forwarding e tracing; checksum/fingerprint coprono action e risorse; final Published/lease revalidation adiacente a `SendSoapAsync`; race rotate/disable/endpoint/action/session a zero rete. | Processo Gateway, provider, store e publisher approvati restano nella TCB; una revoca successiva all'avvio effettivo del dispatch non ritira byte già trasmessi. |
