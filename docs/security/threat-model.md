...
| TM-079 | S/T/I/E | Il connector Sistema TS accetta identità STS, endpoint, adapter, sessione, `Authorization2F`, SOAPAction o operation/profile dal caller; interpreta XML ambiguo; oppure promuove un ID-session senza checkToken nel contesto autenticato. | Modulo allowlisted e Published four-eyes fissano strategy, adapter ID/type, 7/4 binding input, endpoint, Basic, session profile, header/scheme e azione. Gli input scrivono soltanto tramite writer Core e vengono azzerati. Parser verticali impongono QName, sequenza/cardinalità e limiti congelati; DTD, duplicati e nodi inattesi falliscono. Create positivo produce sempre handoff, il candidato UUID passa per checkToken con expiry remota e solo il lifecycle Core esegue la promozione. Composed SOAP riusa la Published A e la generazione promossa. | Gateway/provider/modulo allowlisted restano nella TCB. I payload clinici e i segreti necessari esistono transitoriamente nel processo e sul canale TLS. La prova sintetica non sostituisce accreditamento o live conformance. |
...
`SEC-HC-STS-001` mappa `TM-079` sui test `SistemaTsPublicContractTests`, sul boundary test no-IVT/no
provider/store/host escape e sui due hosted E2E Sistema TS. I test connector-specifici coprono
create/checkToken esatti, binding mancanti, candidato invalido, reject/expiry, DTD/duplicati,
caller endpoint/header/action/identity spoof, redaction, generazione condivisa e contatori wire.
