...
| TM-063 | S/T/E | Il caller promuove P.IVA, payload o `person_id` a subject FSE, oppure sostituisce organizzazione, ruolo, purpose o action. | Il request contract non espone subject/endpoint/auth selector; il profilo Published four-eyes contiene P.IVA e authority, produce CX `FixedSubject` incluso nel checksum; authority esatta Tenant/Application/Installation/Environment/ConnectorVersion/operation; matrice DAP/purpose/action frozen e ricontrollata anche sul contesto workflow. | Publisher/approver collusi o store/host privilegiato restano nella TCB; il profilo human actor non è implementato. |
| TM-064 | T/E | Un JWT manca, usa issuer/audience/temporal profile errato, `nbf` inatteso, x5c/chiave sostituita o firma e mTLS sono usati in modo intercambiabile. | Due header obbligatori; RS256 e leaf x5c da exact signing binding; issuer prefix + CN da DER verificato; fixed audience; `iat+exp` senza `nbf`; binding e purpose distinti per signing/mTLS; revalidation rotation/disable; server sintetico verifica crittograficamente la matrice negativa. | Trust/revocation e custody di certificati ufficiali richiedono qualifica/accreditamento separato. |
| TM-065 | T/I/R | CX/XON ambiguo, hash su bytes normalizzati, workflow context manomesso o RFC7807/risposta upstream espone payload clinico, token o stack. | Formatter FSE2 strict/canonical, controllo normalizzazione e injection; SHA-256 sui bytes exact; workflow store tecnico e role/purpose/action revalidation; response allowlist, bounded parsing e stable safe code senza detail/inner. | Dump privilegiati e telemetria futura restano soggetti a nuova review; nessun payload clinico è persistito da questo pack. |
| TM-066 | E/D | Endpoint caller-controlled, FHIR test-only promosso in produzione, redirect/SSRF, risposta unbounded o retry automatico di mutazioni causa esfiltrazione o duplicazione. | Endpoint HTTPS `/v1` e path/method allowlist da Published profile; test-only fail closed in Production; purpose-bound sender passa solo da restricted transport con DNS/IP, no redirect/proxy, timeout e response bound; solo workflow/trace sono `SAFE_RETRY`. | Disponibilità dell'upstream e accreditamento ufficiale non sono provati dal server sintetico. |
