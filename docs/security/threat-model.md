...
| TM-046 | S/T/I | OAuth outbound subisce state fixation, code replay, callback cross-context o token vending verso il caller. | State casuale conservato solo come hash, attempt breve e one-time, binding a contesto/versioni/revisioni, token-session reference opaca e bearer applicato solo server-side. | Un trusted presentation adapter o il processo Gateway compromesso restano nella TCB. |
| TM-047 | I/E/D | Token endpoint, refresh o cache vengono usati per SSRF, redirect, scope escalation, stale token dopo rotation/disable o leakage nei log/errori. | Endpoint/scopes/audience server-owned, DNS/IP policy e `IRestrictedTransport`, redirect deny, cache bounded con security stamp completo, refresh single-flight/no stale fallback, audit allowlist e failure code sanificato. | Cache in-process perde le sessioni al restart; scale-out e revocation endpoint richiedono una decisione futura, non Redis implicito. |
