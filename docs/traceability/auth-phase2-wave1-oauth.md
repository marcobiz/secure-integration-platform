...
| Authorization Code compatibility with no PKCE | `W1_IT_PKCE_S256_is_server_generated_bound_single_use_and_NONE_remains_compatible`; existing OAuth regression | PASS local |
...
| Shared bounded cache, complete key and single-flight | `W1_IT_Client_credentials_cache_is_shared_revision_bound_and_single_flight`; cache architecture test | PASS local |
| Rotate/disable/stale denial | `W1_SEC_Client_credentials_disabled_rotated_stale_cache_and_SSRF_fail_before_dispatch`; existing refresh tombstone test | PASS local |
