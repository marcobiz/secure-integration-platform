...
| Authoritative organization subject and approval | Dynamically loaded `Fse2OrganizationExecutionModule`; immutable `AuthorizedPublishedExtensionConfiguration` from exact initially-authorized Published A; P.IVA+assigning authority → canonical fixed-subject CX; canonical hosted flow includes editor, distinct approver, publication, BGW1 and grant | PASS local + PostgreSQL 18 / independent review pending |
...
| Authorized dual signing slots | FSE2 extension selects exact vertical slots `authorization` and `integrity`; Core exact-matches Published A, enforces once-per-slot and applies Bearer plus `FSE-JWT-Signature`; unknown and repeated slot negatives have network zero | PASS local |
...
| Published-A final authority | Core capability dispatcher retains the initial opaque Published stamp for both signs and transport; real FSE2 race publishes B during second-slot preparation and asserts stale denial, FSE2 network 0 and generic transport 0 | PASS local |
| Payload/catalog/CX/XON/OID hardening | Defensive caller snapshot; deterministic final multipart bytes feed both digest and body-only transport request; frozen/immutable catalog and canonical CX/XON/OID vectors; server recomputes digest over network bytes | PASS local |
| Workflow correlation scope | Module-owned technical-only store uses Tenant/Application/Installation/Environment/ConnectorVersion/exact profile checksum/origin; no patient or document persistence; durable cross-process workflow persistence is not claimed | PASS local / durability explicitly out of scope |
| Restricted destination and authority | Endpoint, method, content type, certificate, DNS/private-destination policy, timeout and response bound remain exact Published Core authority; attacker metadata cannot select them; success has one outbound/no retry | PASS local |
| Canonical hosted PostgreSQL 18 FSE2 path | `FSE2_IT_PRODUCTION_HOST_PostgreSQL18_Published_Organization_dual_JWT_mTLS_exact_bytes`; migrations through `0013`, real store/four-eyes/BGW1/grant/module/HTTPS | PASS local on ephemeral PostgreSQL 18; no official call |
