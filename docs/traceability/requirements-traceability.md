...
| Guided Connector onboarding | `M5_UT_guided_endpoint_catalog_resolves_only_the_exact_server_owned_scope_and_revision`, `M5_UT_guided_binding_resolves_endpoint_and_provider_catalog_assertions_server_side`, `UI-MOCK-12`, `UI-MOCK-38`, `FULLSTACK-02` | `AUTOMATED` — PASS PostgreSQL 18 + produzione Admin build; 5 azioni, 3 sessioni distinte, file upload normale, handoff monouso, failure dopo binding + reload/resume, binding/grant singoli, Published e invocation 200; 0×429/re-login/wait/support/duplicati |
...
| Workflow correlation scope | Canonical common-only `SharedOrganizationProfileChecksumSha256` plus Tenant/Application/Installation/Environment/ConnectorVersion/Connector ID drives lookup; separate originating `OperationProfileChecksumSha256` is retained for validation/audit. Hosted create→workflow and create→trace pass; cross-scope and unknown-ID negatives pass; no patient/document persistence and durable cross-process persistence is not claimed | PASS local / durability explicitly out of scope |
