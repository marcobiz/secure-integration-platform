...
| Shared Gateway AUTH/API rate-limit partition and same-NAT UX | Twenty-four exact `ADMIN_RATE_LIMIT_*` cases plus `ADMIN_SESSION_idempotent_role_assignment_preserves_existing_sessions_but_privilege_change_revokes_them`, the PostgreSQL session mutation coverage, `PROVISIONER_clean_state_golden_path_stays_below_25_percent_of_each_quota` and `PROVISIONER_resume_reuses_sessions_and_reaches_Published_Active`: exact semantic AUTH/API classification, typed RemoteIp/AuthenticatedSubject identity, first-request poisoning denial, untrusted forwarded-header denial, DevelopmentApiKey server-owned identity, OIDC pre/post-auth partitioning, safe 429 and exact 60/600/one-minute/queue-zero defaults. Seven host-level same-NAT FQNs prove two sequential supported bootstraps and complete lifecycles on one Gateway/limiter/database/window, six independent sessions/cookie jars, distinct Installation/Environment/Published state, exact per-session login count and no internal-store/SQL harness setup. | Candidate `AUTOMATED`: request 61 and 601 are denied without affecting another IP/subject; the probative PostgreSQL gate reaches Published/Active twice with AUTH=12, zero 429 and no wait/reset/rollover/re-login/support. The previous shared-three-session/internal-bootstrap attestation is explicitly invalidated. Exact duplicate role assignment preserves sessions; a real privilege mutation still revokes them. Clean-state uses at most 25% of each quota. No tenant/Installation partition claim, external IdP limit claim, OfficialTest traffic or real FSE2 material. |
