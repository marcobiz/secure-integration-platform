...
| Bounded upstream and transport diagnostics | Named HTTP 400/503, structured Content-Type, malformed/oversized, `Content-Length`, streaming-limit, caller-cancellation, phase, redirect and non-FSE compatibility tests. Lifecycle is DNS/TCP/TLS/header/body/completed; mTLS requires structural pre-header evidence, post-header reset and ambiguous TLS collapse to other. Bounded FSE2 3xx is not followed and crosses the mapper without `Location`; legacy stays redirect-denied. Mapper retains only official allowlisted code, audit only safe phase/status/category/code, caller Problem remains generic; representative and redirect failures each assert failure audit 1 / success audit 0. | PASS targeted local; raw response absent; exact-head CI pending |
