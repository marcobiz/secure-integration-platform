...
| Shared Gateway AUTH/API rate-limit partition, role convergence and same-NAT UX | Exact `ADMIN_RATE_LIMIT_*`, `ADMIN_ROLE_*`, `FSE2_OFFICIALTEST_dual_same_NAT_*`, PostgreSQL session/race, clean-state and resumability cases: exact semantic AUTH/API classification, typed RemoteIp/AuthenticatedSubject identity, first-request poisoning denial, untrusted forwarded-header denial, DevelopmentApiKey server-owned identity, OIDC pre/post-auth partitioning, safe 429 and exact 60/600/one-minute/queue-zero defaults. The gate observes actual limiter leases (allowed/rejected, partition kind/HMAC, limiter instance and monotonic window generation) and drives both golden paths through `OfficialTestProvisioner` plan/configure/grant/propose/publish/verify; direct Admin mutations are restricted to denial probes. PostgreSQL `READ COMMITTED` insert/conflict/read-back converges exact concurrent tuples without retry, duplicate audit or duplicate revocation. Exact existing self-assignment is an authorized no-op; any privilege expansion is denied. | Candidate `AUTOMATED`: request 61 and 601 remain isolated; the probative PostgreSQL gate reaches two distinct Installation/Environment/binding/grant Published/Active states on one Gateway/database/limiter/window with six cookie jars, AUTH=12 derived from leases, one normalized remote-IP partition, one limiter instance/generation, zero 429/wait/reset/rollover/re-login/support. The prior telemetry/manual-provisioner supplement is invalidated. Concurrent first insert is 2xx/2xx with one row/audit/revocation; concurrent existing no-ops preserve revision, session and expiry. No raw identity, tenant/Installation partition claim, OfficialTest traffic or real FSE2 material. |
