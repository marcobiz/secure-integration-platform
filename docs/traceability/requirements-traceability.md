...
| Bounded upstream and transport diagnostics | Named HTTP 400/503, structured Content-Type, malformed/oversized, `Content-Length`, streaming-limit, caller-cancellation, phase, redirect and non-FSE compatibility tests. Lifecycle is DNS/TCP/TLS/header/body/completed; mTLS requires structural pre-header evidence, post-header reset and ambiguous TLS collapse to other. Bounded FSE2 3xx is not followed and crosses the mapper without `Location`; legacy stays redirect-denied. Mapper retains only official allowlisted code; local response mapping failure is a distinct phase with only bounded status and `FSE2_RESPONSE_INVALID`. Audit persistence uses five constrained columns. `FSE2_ADMIN_AUDIT_SecurityAdministrator_reads_bounded_failure_diagnostics`, the non-security negative, OpenAPI parity, PostgreSQL 18 round-trip, caller Problem negative, closed phase matrix, exactly-one audit and evidence reducer tests cover transport → audit → DB → Admin API/client/UI → redacted evidence. | PASS targeted local; raw response absent; PostgreSQL/exact-head CI pending |
