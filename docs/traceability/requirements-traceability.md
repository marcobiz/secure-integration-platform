...
| Exact Published operation policy | `Fse2OrganizationPublishedOperationExpectationProvider` derives mTLS/restricted transport, exact `authorization`+`integrity` slots, RS256/chain x5c, explicit `ContentCommitment` Key Usage on both slots, projections, environment audience, canonical CX, issuer/CN prefixes, 300-second iat-exp/no-nbf/jti, exact claims, equal signing identity and distinction from mTLS; real-Published subject/audience/issuer/projection/missing-extra/unknown slot/temporal/lifetime/x5c/Key-Usage/claim/certificate-relation mismatches use a strategy sentinel and assert signing/DNS/transport/network zero | PASS targeted local and exact-candidate CI 21/21 |
