...
| Exact Published operation policy | `Fse2OrganizationPublishedOperationExpectationProvider` derives mTLS/restricted transport, exact `authorization`+`integrity` slots, RS256, explicit `ContentCommitment` Key Usage on both slots, projections, environment audience, canonical CX, issuer/CN prefixes, 300-second iat-exp/no-nbf/jti, exact claims, equal signing identity and distinction from mTLS. OfficialTest `validate-cda@1.0.1` requires leaf-only x5c for both slots; other profiles retain chain x5c. Real-Published policy mismatches fail before signing/DNS/transport/network. | PASS targeted local; exact-head CI pending |
...
| OfficialTest single-operation canonical source and dry-run | Embedded `fse2-officialtest-validate-cda.connector.json`; `FSE2_OFFICIALTEST_canonical_definition_bytes_checksum_and_single_operation_are_frozen`, `FSE2_OFFICIALTEST_plan_is_redacted_and_has_zero_writes_signing_DNS_HTTPS_transport_and_network`, `FSE2_OFFICIALTEST_validate_cda_VERIFICA_omits_mode_and_attachment_hash`, `FSE2_OFFICIALTEST_validate_cda_request_matches_frozen_ministerial_contract`, `FSE2_OFFICIALTEST_UT_rooted_operation_path_preserves_the_complete_server_owned_base_prefix_without_network`, `FSE2_OFFICIALTEST_HOSTED_authoritative_runtime_recheck_preserves_effective_URI_and_dispatches_once_to_local_mock`, `FSE2_OFFICIALTEST_RUNTIME_stale_base_or_operation_path_denies_before_signing_DNS_HTTPS_and_transport`, endpoint/caller-authority negatives | PASS targeted local: exact runtime URI, frozen two-field ministerial body, one controlled local mock dispatch with dual JWT/A1 mTLS, no `mode`/`attachment_hash`; stale/malformed URI counters remain zero; real configuration and live call absent; exact-head CI pending |
