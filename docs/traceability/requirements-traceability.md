...
| Shared Gateway AUTH/API rate-limit partition | Ten exact `ADMIN_RATE_LIMIT_*` cases plus `PROVISIONER_clean_state_completes_under_default_limits_without_429`: typed AUTH/API and RemoteIp/AuthenticatedSubject identity, both first-request orders, cross-policy/principal/tenant isolation, untrusted forwarded-header denial, bounded safe 429 and exact 20/240/one-minute/queue-zero defaults | Candidate `AUTOMATED`: no threshold increase or disablement; reduced-limit fixture has target 429 >0 with other-principal/cross-policy/cross-tenant rejection zero; real clean-state uses defaults and must retain positive headroom, zero 429 and Published/Active. No OfficialTest traffic or real FSE2 material. |
