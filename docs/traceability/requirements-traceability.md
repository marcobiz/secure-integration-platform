...
| Four-eyes/semantic approval | `M5_UT_Approval_review_is_semantic_canonical_and_contains_no_credential_value`, `M5_UT_Approval_digest_covers_every_catalog_and_certificate_revision_dimension`, PG18 transactional race, integrated anti-exfiltration test, `UI-MOCK-29/33`, `FULLSTACK-01`; exact catalog/metadata/resource/certificate/binding revisions and digest equality | PASS local; exact-head CI pending |
| Provider resource reference safety | Structured `ProviderResourceReference`; `M5_IT_DAT_PostgreSQL18_runtime_locator_is_exactly_granted_and_not_enumerable_when_configured`; controlled runtime function; direct/enumerated locator denial; wrong operation/environment/revision/status denial; M4 quickstart uses a distinct non-superuser Admin pool | PASS 10/10 PG18 and M4 quickstart; exact-head CI pending |
...
| i18n/theme/a11y | Backend `runtime-wire-codes.json`, embedded authenticated export, generated typed frontend contract, exact IT/EN parity and known-code coverage; 28 Vitest; `UI-MOCK-16/17/18/20/22/25/28/30/31/32/33`; axe critical/serious = 0 | PASS local; exact-head CI pending |
