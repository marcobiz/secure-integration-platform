...
| Exact Published operation policy | `Fse2OrganizationPublishedOperationExpectationProvider` derives mTLS/restricted transport, exact `authorization`+`integrity` slots, RS256/chain x5c, projections, environment audience, canonical CX, issuer/CN prefixes, 300-second iat-exp/no-nbf/jti, exact claims, equal signing identity and distinction from mTLS; real-Published subject/audience/issuer/projection/missing-extra-unknown slot/temporal/lifetime/x5c/claim/certificate-relation mismatches use a strategy sentinel and assert signing/transport/network zero | PASS local / focused Core zero-scope and zero-DNS regression retained |
...
| Published-A final authority | Core capability dispatcher retains the initial opaque Published stamp for preflight, both signs, composed request and transport; real FSE2 preflight race publishes B and asserts stale denial with strategy/signing/FSE2 network/generic transport zero; focused Core races retain before-sign, post-composition/pre-transport, post-DNS, stale and expiry coverage | PASS local / exact focused suite gate pending final head |
