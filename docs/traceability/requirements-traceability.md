...
| Authoritative organization subject and approval | `PublishedConnectorFse2ProfileResolver` over `IConnectorConfigurationStore` + `PublishedConnectorAccessContext` and exact current `IAdminSecurityStore` approval/binding digest; invalidated-approval and caller-surface production-path tests; resolved profile constructor is internal | PASS local / targeted re-review pending |
| Claim authority and role coherence | Frozen operation theory; DAP/purpose/action validation; production-path fixed organization subject remains distinct from `person_id`; workflow origin tuple is revalidated | PASS local |
| Published signing/mTLS resources | Resolver projects actual logical ID/provider reference/catalog revision+checksum/binding revision+checksum and approved fingerprint/SPKI/version; stable substitution and cross-use connector tests assert network zero | PASS local |
| Composite final authority | Same `Fse2DispatchLease` supplies JWT1/JWT2/signing/mTLS/endpoint; final full Published re-read after mTLS resolution and before synchronous header projection; signing, mTLS, organization and endpoint TCS races assert network zero | PASS local |
| Payload/catalog/CN/OID hardening | Invocation-entry private byte snapshot feeds validation/hash/body; getter/input/await mutation test; Frozen/Immutable catalog tests; DER X.500 exact-CN matrix; semantic ASN.1 OID vectors | PASS local |
| Workflow full authority | Immutable Tenant/Application/Installation/Environment/ConnectorVersion/profile revision+checksum/origin key; only workflow/trace technical identifiers persisted; cross-context matrix asserts network zero | PASS local |
| Restricted destination and authority | Exact official Production/OfficialTest origin+base path and default port; test-only synthetic authority; wrong grant/test-only/attacker endpoint/endpoint race assert provider or network zero | PASS local |
| Synthetic HTTPS/mTLS conformance | Per-run CA/server/signing/mTLS certificates; real mTLS, RS256 verification, both headers, leaf x5c, exact issuer CN, fixed sub and exact file hash; `SERVER_VALIDATION_TEST` malformed/missing-header negatives are reported separately from connector security tests | PASS local; no official call |
