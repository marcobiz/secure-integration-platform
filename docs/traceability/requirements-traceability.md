...
| Payload/catalog/CX/XON/OID hardening | `FSE2_HASH_validate_cda_omits_claim_and_publication_uses_exact_input_file_bytes_not_multipart`; `FSE2_IT_all_11_operations_use_Published_paths_body_modes_and_exact_input_file_hash_contract`; defensive caller snapshot; frozen/immutable catalog and canonical CX/XON/OID vectors | PASS targeted local: `attachment_hash` uses exact input-file bytes only where required, `validate-cda` omits it, and the all-operation server rejects the multipart-envelope digest; exact-head CI/review pending |
