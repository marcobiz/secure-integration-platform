...
| OfficialTest supported provisioning/four-eyes | `FSE2_OFFICIALTEST_supported_provisioner_configures_proposes_approves_and_reads_back_exact_configuration`; `FSE2_OFFICIALTEST_ARCH_external_metadata_file_or_mismatch_cannot_be_authority_and_Admin_API_catalog_is_required`; `FSE2_OFFICIALTEST_UT_exact_provider_lookup_is_bounded_current_and_independent_of_page_size_or_concurrent_catalog_mutation`; dedicated `FSE2_OFFICIALTEST_PostgreSQL18_D1_approval_is_invalidated_before_D2_distinct_approver_publication_and_replay_is_denied` performs the real publish-as-A denial then publish-as-B | Exact lookup is non-paginated; PostgreSQL/API gate must pass with zero skip before publication claim |
