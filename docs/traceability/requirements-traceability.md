...
| Local no-cloud provider boundary | `BrokerGateway.LocalPkcs12.slnx`; optional pack under `packs/deployment/local-pkcs12`; exact logical references, closed manifest, no caller path/key selection, ephemeral P12 load and no private export; Core/default Gateway remain pack-independent | PASS synthetic provider tests; real material not accessed |
| Offline import and custody gate | `New-Fse2LocalPkcs12Material.ps1` defaults read-only and requires exact out-of-band A1/S1/root fingerprints, SPKI match, distinct roles, direct trust and new external output before explicit `-Execute`; synthetic self-test covers create and mismatch cleanup | PASS synthetic; operational import NOT PERFORMED |
| Local Compose qualification | Opt-in overlay and `Invoke-Fse2LocalProviderLab.ps1` first complete the canonical Synthetic-provider quickstart, then recreate only Gateway with server-owned A1/S1 bindings; default quickstart unchanged; image is non-root/read-only with read-only material mounts | PASS local with synthetic import/start/stop and cleanup; exact-head CI pending; LIVE_FSE2_CALLS=0 |
