...
| Bounded upstream and transport diagnostics | Named HTTP 400/503/malformed/oversized/non-FSE compatibility tests plus hosted HTTP/TLS/mTLS/TCP and unit DNS/timeout/other matrices; mapper retains only official allowlisted code, audit only safe phase/status/category/code, caller Problem remains generic | PASS targeted local; raw response absent; exact-head CI pending |
| Frozen validate-cda case 476 contract | `FSE2_LIVE_FIXTURE_matches_frozen_validate_cda_contract_without_network` pins official/dataset commits and dataset SHA, POST path, `VERIFICA`/`CDA`/`ATTACHMENT`, two multipart parts and absent `attachment_hash`; hosted wire proves Core-owned endpoint/auth/signature/Accept and retry/redirect zero | PASS offline contract; dataset byte identity remains external T03 evidence; OfficialTest DNS/network zero |
