...
| Shared Gateway AUTH/API rate-limit partition and same-NAT UX | Twelve exact `ADMIN_RATE_LIMIT_*` cases plus `PROVISIONER_clean_state_golden_path_stays_below_25_percent_of_each_quota` and `PROVISIONER_resume_reuses_sessions_and_reaches_Published_Active`: exact semantic AUTH/API classification, typed RemoteIp/AuthenticatedSubject identity, first-request poisoning denial, untrusted forwarded-header denial, DevelopmentAuth same-NAT double onboarding, DevelopmentApiKey server-owned identity, OIDC pre/post-auth partitioning, safe 429 and exact 60/600/one-minute/queue-zero defaults | Candidate `AUTOMATED`: request 61 and 601 are denied without affecting another IP/subject; two PostgreSQL DevelopmentAuth onboarding workflows reuse one distinct cookie jar per role and reach Published/Active with zero 429, no wait/reset/relogin. Clean-state uses at most 25% of each quota. No tenant/Installation partition claim, external IdP limit claim, OfficialTest traffic or real FSE2 material. |
