...
| HC-18 | Umbria FSE | **KNOWN** REST + JSON | **KNOWN** two RS256 JWTs are generated/used before the call: bearer access token and `FSE-JWT-Signature`; mTLS. Lifetime, reuse and regeneration frequency are **UNKNOWN** | **KNOWN** two pharmacy-specific certificates, one for mTLS and one for signing; key custody and centralizability are **NEEDS CHARACTERIZATION** |
