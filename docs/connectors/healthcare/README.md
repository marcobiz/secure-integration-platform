...
`KNOWN` is a historical characterization label, not current official or live-verified evidence. The Sistema TS Wave 1 source questions are resolved from current public material and the fixed `Authorization2F` projection is integrated. Implementation remains `BLOCKED_BY_GENERIC_PRIMITIVE` because the current lifecycle cannot promote the production ID-session delivered out of band or compose the mandatory SOAP 1.1 `SOAPAction` with that projection in one restricted dispatch. Other production profiles remain `NEEDS_PUBLIC_SOURCE` and NO-GO until their own unresolved questions are closed.
