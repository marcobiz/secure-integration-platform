...
function Get-SanitizedErrorCode {
    ...
    return 'M3A_SPLIT_VM_OPERATOR_FAILURE'
}
...
try {
    ...
    $worktree = (& git.exe -c ('safe.directory=' + $safeRepository) -C $RepositoryRoot status --porcelain | Out-String).Trim()
    if ($LASTEXITCODE -ne 0 -or $worktree) { throw 'M3A_SPLIT_VM_OPERATOR_WORKTREE_NOT_CLEAN.' }
    & git.exe -c ('safe.directory=' + $safeRepository) -C $RepositoryRoot fetch --prune origin *> $null
    if ($LASTEXITCODE -ne 0) { throw 'M3A_SPLIT_VM_OPERATOR_FETCH_FAILED.' }
    & git.exe -c ('safe.directory=' + $safeRepository) -C $RepositoryRoot cat-file -e ($candidateCommit + '^{commit}') *> $null
    if ($LASTEXITCODE -ne 0) { throw 'M3A_SPLIT_VM_OPERATOR_COMMIT_MISSING.' }
    & git.exe -c ('safe.directory=' + $safeRepository) -C $RepositoryRoot switch --detach $candidateCommit *> $null
    if ($LASTEXITCODE -ne 0 -or (& git.exe -c ('safe.directory=' + $safeRepository) -C $RepositoryRoot rev-parse HEAD).Trim() -ne $candidateCommit) {
        throw 'M3A_SPLIT_VM_OPERATOR_HEAD_MISMATCH.'
    }
    ...
    $errorCode = Get-SanitizedErrorCode -ErrorRecord $_
