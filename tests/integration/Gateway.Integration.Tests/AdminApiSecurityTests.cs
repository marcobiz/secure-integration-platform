...
public sealed class AdminApiSecurityTests
{
    ...
    internal static async Task<SameNatOnboardingResult> RunTwoDevelopmentAuthOnboardingsPostgreSqlAsync()
    {
        string? adminConnectionString = Environment.GetEnvironmentVariable("GATEWAY_POSTGRES_ADMIN_CONNECTION");
        string? migrationConnectionString = Environment.GetEnvironmentVariable("GATEWAY_POSTGRES_MIGRATION_CONNECTION");
        if (string.IsNullOrWhiteSpace(adminConnectionString) || string.IsNullOrWhiteSpace(migrationConnectionString))
            Assert.Skip("The same-NAT onboarding gate requires the dedicated PostgreSQL 18 job.");
        await using PostgresRuntimeRoleLease runtimeRole = await PostgresRuntimeRoleLease.CreateAsync(
            adminConnectionString,
            migrationConnectionString,
            TestContext.Current.CancellationToken);
        await using SameNatOnboardingFactory factory = new(runtimeRole.ConnectionString, adminConnectionString);
        return await RunTwoDevelopmentAuthOnboardingsAsync(factory);
    }

    private static async Task<SameNatOnboardingResult> RunTwoDevelopmentAuthOnboardingsAsync(
        SameNatOnboardingFactory factory)
    {
        using TrackedAdminSession securityAdministrator =
            await TrackedAdminSession.LoginAsync(factory, "security-admin", TestContext.Current.CancellationToken);
        using TrackedAdminSession editor =
            await TrackedAdminSession.LoginAsync(factory, "editor", TestContext.Current.CancellationToken);
        using TrackedAdminSession approver =
            await TrackedAdminSession.LoginAsync(factory, "approver", TestContext.Current.CancellationToken);

        string runSuffix = Guid.NewGuid().ToString("N")[..12];
        Guid environmentId = Guid.NewGuid();
        await factory.Services.GetRequiredService<IAdminGatewayRegistry>().AddEnvironmentAsync(
            new(environmentId, "same-nat-" + runSuffix, "Same NAT onboarding " + runSuffix, false),
            TestContext.Current.CancellationToken);

        string[] connectorIds =
        [
            $"same-nat-a-{runSuffix}",
            $"same-nat-b-{runSuffix}"
        ];
        List<string> states = [];
        foreach (string connectorId in connectorIds)
        {
            states.Add(await CompleteDevelopmentAuthOnboardingAsync(
                factory,
        ...
        Dictionary<string, int> authCounts = new(StringComparer.Ordinal)
        {
            ["security-admin"] = securityAdministrator.AuthRequestCount,
            ["editor"] = editor.AuthRequestCount,
            ["approver"] = approver.AuthRequestCount
        };
        Dictionary<string, int> apiCounts = new(StringComparer.Ordinal)
        {
            ["security-admin"] = securityAdministrator.ApiRequestCount,
            ["editor"] = editor.ApiRequestCount,
            ["approver"] = approver.ApiRequestCount
        };
        Dictionary<string, int> loginCounts = new(StringComparer.Ordinal)
        {
            ["security-admin"] = securityAdministrator.LoginCount,
            ["editor"] = editor.LoginCount,
            ["approver"] = approver.LoginCount
        };
        Dictionary<string, int> csrfCounts = new(StringComparer.Ordinal)
        {
            ["security-admin"] = securityAdministrator.CsrfAcquisitionCount,
            ["editor"] = editor.CsrfAcquisitionCount,
            ["approver"] = approver.CsrfAcquisitionCount
        };
        int rejectionCount = securityAdministrator.RateLimitRejectionCount +
            editor.RateLimitRejectionCount +
            approver.RateLimitRejectionCount;
        bool distinctCookieJars = new[]
        {
            securityAdministrator.SessionCookie,
            editor.SessionCookie,
            approver.SessionCookie
        }.Distinct(StringComparer.Ordinal).Count() == 3;

        TestContext.Current.TestOutputHelper?.WriteLine(
            "SAME_NAT_ONBOARDING_COUNTS " + JsonSerializer.Serialize(new
            {
                authRequestCountByRole = authCounts,
                apiRequestCountByRole = apiCounts,
                authRequestCount = authCounts.Values.Sum(),
                maximumApiRequestCountPerSubject = apiCounts.Values.Max(),
                rateLimitRejectionCount = rejectionCount,
                configurationStates = states,
                loginCountByRole = loginCounts,
                csrfAcquisitionCountByRole = csrfCounts,
                distinctCookieJars
            }, WireJson));

        return new(
            states,
            authCounts.Values.Sum(),
            apiCounts,
            rejectionCount,
            loginCounts,
            csrfCounts,
    ...
    private static async Task<string> CompleteDevelopmentAuthOnboardingAsync(
        SameNatOnboardingFactory factory,
        TrackedAdminSession securityAdministrator,
        TrackedAdminSession editor,
        TrackedAdminSession approver,
        Guid environmentId,
        string connectorId,
        CancellationToken cancellationToken)
    {
        using HttpResponseMessage dashboard = await securityAdministrator.SendAsync(
            HttpMethod.Get,
            "/admin/api/v1/dashboard",
            cancellationToken: cancellationToken);
        dashboard.EnsureSuccessStatusCode();

        using HttpResponseMessage sampleResponse = await editor.SendAsync(
            HttpMethod.Get,
            "/admin/api/v1/connectors/sample",
            cancellationToken: cancellationToken);
        sampleResponse.EnsureSuccessStatusCode();
        using JsonDocument sample = await JsonDocument.ParseAsync(
            await sampleResponse.Content.ReadAsStreamAsync(cancellationToken),
            cancellationToken: cancellationToken);
        using JsonDocument definition = JsonDocument.Parse(
            sample.RootElement.GetRawText().Replace(
                "sample-secure-service",
                connectorId,
                StringComparison.Ordinal));

        using HttpResponseMessage validationResponse = await editor.SendAsync(
            HttpMethod.Post,
            "/admin/api/v1/connectors:validate",
            new ConnectorImportRequest(definition.RootElement.Clone()),
            includeCsrf: true,
            cancellationToken: cancellationToken);
        validationResponse.EnsureSuccessStatusCode();
        ConnectorValidationResult validation =
            (await validationResponse.Content.ReadFromJsonAsync<ConnectorValidationResult>(cancellationToken: cancellationToken))!;

        using HttpResponseMessage importResponse = await editor.SendAsync(
            HttpMethod.Post,
            "/admin/api/v1/connectors:import",
            new ConnectorImportRequest(definition.RootElement.Clone(), validation.ChecksumSha256),
            includeCsrf: true,
            cancellationToken: cancellationToken);
        Assert.Equal(HttpStatusCode.Created, importResponse.StatusCode);
        ConnectorVersionResource draft =
            (await importResponse.Content.ReadFromJsonAsync<ConnectorVersionResource>(WireJson, cancellationToken))!;

        using HttpResponseMessage validatedResponse = await editor.SendAsync(
            HttpMethod.Post,
            $"/admin/api/v1/connectors/{connectorId}/versions/{draft.Version}:validate",
            includeCsrf: true,
            ifMatch: $"\"{draft.RowVersion}\"",
            cancellationToken: cancellationToken);
        validatedResponse.EnsureSuccessStatusCode();
        ConnectorVersionResource validated =
            (await validatedResponse.Content.ReadFromJsonAsync<ConnectorVersionResource>(WireJson, cancellationToken))!;

        await RegisterCatalogResourcesAsync(factory, environmentId, connectorId);
        object binding = new
        {
            environmentId,
            connectorVersion = validated.Version,
            ...
            {
                ["sample-vendor-client-certificate"] = new
                {
                    providerId = "synthetic",
                    resourceId = "certificate",
                    resourceType = "ClientCertificate",
                    publicMetadataRevision = 1
                }
            }
        };
        using HttpResponseMessage bindingResponse = await securityAdministrator.SendAsync(
            HttpMethod.Put,
        ...
        using HttpResponseMessage requestResponse = await editor.SendAsync(
            HttpMethod.Post,
            $"/admin/api/v1/connectors/{connectorId}/versions/{validated.Version}/approval-requests",
            includeCsrf: true,
            cancellationToken: cancellationToken);
        requestResponse.EnsureSuccessStatusCode();
        JsonElement approvalRequest =
            await requestResponse.Content.ReadFromJsonAsync<JsonElement>(cancellationToken: cancellationToken);

        using HttpResponseMessage reviewResponse = await editor.SendAsync(
            HttpMethod.Get,
            $"/admin/api/v1/connectors/{connectorId}/versions/{validated.Version}/approval-review",
            cancellationToken: cancellationToken);
        reviewResponse.EnsureSuccessStatusCode();
        JsonElement review =
            await reviewResponse.Content.ReadFromJsonAsync<JsonElement>(cancellationToken: cancellationToken);

        using HttpResponseMessage approvalResponse = await approver.SendAsync(
            HttpMethod.Post,
            $"/admin/api/v1/connectors/{connectorId}/versions/{validated.Version}/approvals",
            new
            {
        ...
        using HttpResponseMessage publishResponse = await approver.SendAsync(
            HttpMethod.Post,
            $"/admin/api/v1/connectors/{connectorId}/versions/{validated.Version}:publish",
            new
            {
            ...
            includeCsrf: true,
            ifMatch: $"\"{validated.RowVersion}\"",
            cancellationToken: cancellationToken);
        Assert.True(
            publishResponse.IsSuccessStatusCode,
            $"Synthetic publish failed with {(int)publishResponse.StatusCode}: " +
            await publishResponse.Content.ReadAsStringAsync(cancellationToken));

        using HttpResponseMessage finalVersionResponse = await approver.SendAsync(
            HttpMethod.Get,
            $"/admin/api/v1/connectors/{connectorId}/versions/{validated.Version}",
            cancellationToken: cancellationToken);
        finalVersionResponse.EnsureSuccessStatusCode();
        JsonElement finalVersion =
            await finalVersionResponse.Content.ReadFromJsonAsync<JsonElement>(cancellationToken: cancellationToken);

        using HttpResponseMessage finalBindingsResponse = await approver.SendAsync(
            HttpMethod.Get,
            $"/admin/api/v1/connectors/{connectorId}/versions/{validated.Version}/bindings?environmentId={environmentId:D}&offset=0&limit=10",
            cancellationToken: cancellationToken);
        finalBindingsResponse.EnsureSuccessStatusCode();
        JsonElement finalBindings =
            await finalBindingsResponse.Content.ReadFromJsonAsync<JsonElement>(cancellationToken: cancellationToken);
        JsonElement finalBinding = Assert.Single(finalBindings.GetProperty("items").EnumerateArray().ToArray());

        return finalVersion.GetProperty("state").GetString() + "/" +
    ...
    internal sealed record SameNatOnboardingResult(
        IReadOnlyList<string> ConfigurationStates,
        int AuthRequestCount,
        IReadOnlyDictionary<string, int> ApiRequestCounts,
        int RateLimitRejectionCount,
        IReadOnlyDictionary<string, int> LoginCounts,
        IReadOnlyDictionary<string, int> CsrfAcquisitionCounts,
        bool DistinctCookieJars);

    private sealed class TrackedAdminSession(HttpClient client) : IDisposable
    {
        ...
        public int CsrfAcquisitionCount { get; private set; }

        internal static async Task<TrackedAdminSession> LoginAsync(
            SameNatOnboardingFactory factory,
            string user,
            CancellationToken cancellationToken)
        {
            ...
            TrackedAdminSession session = new(client);
            session.Csrf = await session.AcquireCsrfAsync(cancellationToken);
            using HttpResponseMessage login = await session.SendAsync(
                HttpMethod.Post,
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            if (includeCsrf) request.Headers.Add("X-CSRF-TOKEN", Csrf);
            if (ifMatch is not null) request.Headers.TryAddWithoutValidation("If-Match", ifMatch);

            if (path is "/admin/auth/login" or "/admin/auth/development/login" ||
                (path == "/admin/auth/csrf" && !authenticated) ||
                (path.StartsWith("/admin/auth/", StringComparison.Ordinal) &&
                 path is not "/admin/auth/csrf" and not "/admin/auth/me" and not "/admin/auth/logout"))
                AuthRequestCount++;
            else if (path.StartsWith("/admin/", StringComparison.Ordinal))
                ApiRequestCount++;

            HttpResponseMessage response = await client.SendAsync(request, cancellationToken);
            if (response.StatusCode == HttpStatusCode.TooManyRequests) RateLimitRejectionCount++;
            return response;
        }

        public void Dispose() => client.Dispose();

        private async Task<string> AcquireCsrfAsync(CancellationToken cancellationToken)
        {
...
{
    [Fact]
    public async Task ADMIN_RATE_LIMIT_two_DevelopmentAuth_onboardings_same_NAT_complete_without_429()
    {
        AdminApiSecurityTests.SameNatOnboardingResult result =
            await AdminApiSecurityTests.RunTwoDevelopmentAuthOnboardingsPostgreSqlAsync();

        Assert.Equal(["Published/Active", "Published/Active"], result.ConfigurationStates);
        Assert.Equal(6, result.AuthRequestCount);
        Assert.Equal(0, result.RateLimitRejectionCount);
        Assert.All(result.ApiRequestCounts.Values, count => Assert.InRange(count, 0, 150));
        Assert.True(result.DistinctCookieJars);
    }

    [Fact]
    public async Task ADMIN_RATE_LIMIT_role_sessions_are_reused_without_duplicate_login()
    {
        AdminApiSecurityTests.SameNatOnboardingResult result =
            await AdminApiSecurityTests.RunTwoDevelopmentAuthOnboardingsPostgreSqlAsync();

        Assert.All(result.LoginCounts.Values, count => Assert.Equal(1, count));
        Assert.All(result.CsrfAcquisitionCounts.Values, count => Assert.Equal(2, count));
        Assert.Equal(6, result.AuthRequestCount);
        Assert.Equal(0, result.RateLimitRejectionCount);
    }
...
public sealed class SameNatOnboardingFactory(
    string runtimeConnectionString,
    string adminConnectionString) : AdminDevelopmentFactory
{
    protected override void ConfigureWebHost(IWebHostBuilder builder)
    {
        base.ConfigureWebHost(builder);
