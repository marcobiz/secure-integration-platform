...
| Wave 1 — Typed SOAP session handshake + authorized external admission | Ultima remediation test/evidence: entrambi i P2 residui chiusi localmente; CI exact-head e targeted re-review pending | PR #23 test/evidence commit `95eeaf5d6e2c2170570f48e7570f90b8dfb4e646`: vero host `Program`/HTTP/BGW1/DI su PostgreSQL, replay cross-Tenant/Application/Installation con grant validi e race deterministica sullo stesso `RoutingConnectorConfigurationStore`; 3/3 nuovi casi, 456 ordinary e 112/112 PostgreSQL integration PASS; production code invariato |
...
- evidenza ultima remediation: 41 unit typed, 8 integration/API typed (inclusi vero host autenticato e due race PostgreSQL), 35 unit SOAP legacy/config, 5 integration SOAP legacy e 24 architecture PASS; suite ordinaria 456 totali (441 PASS, 15 PostgreSQL conditional), PostgreSQL 18 112/112, Admin 28 unit + 37 UI mock + 2 a11y + full-stack 1/1 senza retry, build Release zero warning/errori, document validation, secret scan, Gitleaks sul delta, SBOM e vulnerability scan PASS. Production code invariato; nuovo Core export exact-head, CI del commit documentale e targeted re-review restano gate di handoff, merge non autorizzato;
