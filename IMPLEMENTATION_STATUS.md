...
| Wave 1 — Typed composed SOAP authenticated dispatch | Product gate locale PASS; CI exact-head e review indipendente pending | Basic + SOAP metadata tipizzati + opaque custom header in una invocation Published; 426 ordinary PASS + 12 PG conditional, 12/12 PostgreSQL 18, 92 unit mirati, 20 real-HTTPS regression, 24 architecture e `FULLSTACK-01` PASS; schema/catalog opt-in per `opaqueSessionHttp` e `soapBasicOpaqueSession`; nessun connector production |
...
- Basic viene risolto sul solo request interno; la sessione usa la lease opaque e la cache/generation/expiry M6 già qualificate; Authorization, SOAPAction e Content-Type non sono placement custom valide;
...
- product gate locale PASS: build Release senza warning/errori, 426 test ordinari PASS e 12 PostgreSQL-condizionali SKIP, poi 12/12 PostgreSQL 18 PASS; 28/28 Vitest, build Admin, `FULLSTACK-01`, redazione/cleanup, scan, SBOM, vulnerability e document validation PASS;
