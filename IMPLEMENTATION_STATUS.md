...
| Healthcare Wave 1 - Sistema TS ePrescription | **BLOCKED_BY_GENERIC_PRIMITIVE; implementation audit 2026-08-09** | freeze riconfermato con 7/7 digest invariati; la projection `Authorization2F` è integrata, ma il lifecycle M6 non può inviare i campi server-owned di `create`, validare/promuovere l'ID-sessione consegnato out-of-band né comporre `SOAPAction` SOAP 1.1 nello stesso one-shot dispatch; nessun connector o server sintetico implementato |
