...
| Healthcare Wave 1 - Sistema TS ePrescription | **IMPLEMENTATION_READY; independent connector review and exact-head CI pending** | rebased on qualified baseline `b1810ed`; external module `healthcare-sistema-ts`, exact server-owned STS create/checkToken adapters, authenticated admission into the shared lifecycle and four frozen SSN business operations via Basic + `Authorization2F: Bearer` + SOAP 1.1. Connector-specific hosted BGW1/HTTPS E2E and PostgreSQL 18 four-eyes E2E PASS locally; accreditation/live conformance remain blocked separately |
