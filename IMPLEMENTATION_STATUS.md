...
| Healthcare Wave 1 - Sistema TS ePrescription | **CONNECTOR-LOCAL VALIDATION READY; BUSINESS DISPATCH BLOCKED BY CORE** | external module and exact create/checkToken admission remain enabled; four business operations are absent from the Published sample and fail closed because the current composed SOAP capability cannot combine caller business data with Core-resolved server-owned fields. Exact frozen-shape validators/serializers, all-operation synthetic HTTPS wire tests and mandatory PostgreSQL admission CI are local PASS; `SERVER_OWNED_BUSINESS_FIELDS`, `BUSINESS_SOAP` and `POSTGRESQL_FULL_BUSINESS_E2E` remain `BLOCKED_BY_CORE` |
