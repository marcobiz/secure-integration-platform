...
| Healthcare Wave 1 - Sistema TS ePrescription | **NOT_READY; frozen-surface hard stop 2026-08-10** | exact baseline `3f8667b`: admission, shared lifecycle, composed SOAP ed execution seam sono presenti; il production host non registra adapter verticali dal modulo e il request context non espone i valori STS provider-resolved obbligatori (`userId`, identificativo cifrato, CF e codici organizzativi). Implementare senza una modifica Core aggirerebbe la custodia server-side; nessun connector o server sintetico fuorviante aggiunto |
