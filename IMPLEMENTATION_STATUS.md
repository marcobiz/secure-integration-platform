...
| Healthcare Wave 1 — FSE2 National Connector | **ORGANIZATION_PROFILE: IMPLEMENTATION_READY, review indipendente pending; HUMAN_ACTOR_PROFILE: DEFERRED** | Il gate indipendente ha autorizzato il profilo iniziale organizzativo: P.IVA/CX four-eyes e Published diventa `JwtSubjectPolicy.Fixed`; dual JWT, x5c, firma/mTLS separati, restricted transport e server sintetico HTTPS/mTLS sono implementati. Nessun input business è promosso a `sub`. L'accreditamento ufficiale resta separato e non eseguito; vedere `docs/testing/FSE2-IMPLEMENTATION-REPORT.md`. |
